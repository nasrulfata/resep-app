  ...
  create(data: Resep) {
    const info = this.db
      .prepare(`
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?)
      `)
      .run(data.nama, data.deskripsi, data.porsi, data.waktuBuat, data.totalHarga, data.hargaPerPorsi, data.catatan)
    ...
    for (const bahan of data.bahan) {
      insertBahan.run(resepId, bahan.barangId, bahan.namaBarang, bahan.jumlah, bahan.satuan, bahan.hargaSatuan, bahan.subtotal)
    }
  ...
  update(id: number, data: Partial<Resep>) {
    this.db
      .prepare(`
        ...
        WHERE id = ?
      `)
      .run(data.nama, data.deskripsi, data.porsi, data.waktuBuat, data.totalHarga, data.hargaPerPorsi, data.catatan, id)
    ...
    if (data.bahan) {
      ...
      `)
      for (const bahan of data.bahan) {
        insertBahan.run(id, bahan.barangId, bahan.namaBarang, bahan.jumlah, bahan.satuan, bahan.hargaSatuan, bahan.subtotal)
      }
    }
