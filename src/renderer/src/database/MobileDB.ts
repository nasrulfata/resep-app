...
const runMigrations = async (database: SQLiteDBConnection) => {
  ...
  // Run ALTER TABLE for existing mobile databases
  try { await database.execute("ALTER TABLE barang ADD COLUMN jenis TEXT DEFAULT 'bahan'") } catch(e){}
  try { await database.execute("ALTER TABLE resep ADD COLUMN overhead REAL DEFAULT 0") } catch(e){}
  try { await database.execute("ALTER TABLE resep ADD COLUMN penyusutan REAL DEFAULT 0") } catch(e){}
  try { await database.execute("ALTER TABLE resep ADD COLUMN hargaJual REAL DEFAULT 0") } catch(e){}
  try { await database.execute("ALTER TABLE resep ADD COLUMN keuntungan REAL DEFAULT 0") } catch(e){}
  try { await database.execute("ALTER TABLE resep ADD COLUMN hpp REAL DEFAULT 0") } catch(e){}
  try { await database.execute("ALTER TABLE bahan_resep ADD COLUMN jenis TEXT DEFAULT 'bahan'") } catch(e){}
};
